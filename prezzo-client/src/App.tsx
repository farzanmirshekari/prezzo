...
import { Presentation_State, User_Interface_State } from './interfaces/interface-models'
...
function App() {
    ...
    const [presentation_state, set_presentation_state] = useState<Presentation_State>({
    ...
    const [user_interface_state, set_user_interface_state] = useState<User_Interface_State>({
    ...
    const upload_image = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        form_data.append('presentation_uuid', presentation_state.presentation_uuid)
        axios
    ...
    return (
        <div className="absolute w-full h-full flex flex-row justify-start">
            {!user_interface_state.presentation_mode && (
                <>
                    <Deck presentation_slides={presentation_state.presentation_slides} />
                    <Editor
                        presentation_markdown={presentation_state.presenatation_markdown}
                        existing_presentation_uuid={presentation_state.existing_presentation_uuid}
                        should_take_in_existing_presentation_uuid={user_interface_state.should_take_in_existing_presentation_uuid}
